...
export const SetMeetingModal = ({
  ...
  const { mutate } = useMutation(createMeeting, {
    onSuccess: (data) => {
      const message = "Success creating meeting";
      alert(message);
    },
    onError: (error) => {
      console.error(error);
          ...
          <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
            ...
            <div className="relative p-6 flex-auto">
              <p className="my-4 text-slate-500 text-lg leading-relaxed">
                Meeting Start: {new Date(startTile).toUTCString()}
              </p>
              <p className="my-4 text-slate-500 text-lg leading-relaxed">
                Meeting End: {new Date(endTile).toUTCString()}
              </p>
              <div>
                <label
                ...
                <input
                  ...
                  value={meetingObject}
                  onChange={(input) => {
                    setMeetingObject(input.target.value);
                  }}
                  placeholder="Object"
                />
              </div>
              ...
              <button
                ...
                onClick={() => {
                  mutate({
                    ...
                    startDate: new Date(startTile).toUTCString(),
